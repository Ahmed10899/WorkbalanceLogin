...
document.getElementById('loginForm').addEventListener('submit', function(event) {
    ...
    if (username === "workbalance" && password === "1234") {
        showMessage(translations[document.getElementById('languageSelect').value].successLogin, "success");

        // Login-Bereich ausblenden, Dashboard anzeigen
        document.getElementById('loginSection').style.display = 'none';
        document.getElementById('dashboardSection').style.display = 'block';

        // Aktualisiere die Nachricht im Dashboard
        document.querySelector('.dashboard-container p').textContent = translations[document.getElementById('languageSelect').value].successfulLoginMessage;
    } else {
        showMessage(translations[document.getElementById('languageSelect').value].errorLogin, "error");
    }
});
...
function changeLanguage() {
    ...
    // Bleibdauer auf der Login-Seite
    updateLoginTime(selectedLanguage);
}
